  ...
  return (
    <Box mt={4}>
      ...
      <form onSubmit={handleSubmit}>
        <Grid container spacing={2}>
          {inputs.map((input, idx) => (
            <Grid item xs={12} key={idx}>
              <Paper sx={{ p: 3, borderRadius: 3, boxShadow: '0 4px 24px 0 rgba(255,182,193,0.15)' }}>
                ...
                {input.error && !input.error.includes('URL') && !input.error.includes('Validity') && !input.error.includes('Shortcode') && (
                  <Alert severity="error" sx={{ mt: 2, borderRadius: 2 }}>{input.error}</Alert>
                )}
              </Paper>
            </Grid>
          ))}
        ...
        <Button type="submit" variant="contained" color="primary" sx={{ mt: 4, borderRadius: 3, boxShadow: '0 2px 8px 0 rgba(255,182,193,0.15)', fontWeight: 600, fontSize: '1.1rem', px: 4 }}>
          Shorten URLs
        </Button>
      </form>
      {results.length > 0 && (
        <Box mt={5}>
          <Typography variant="h6" sx={{ mb: 2 }}>Shortened URLs</Typography>
